...
class DownReportManager(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, shell: ShellCore, report_channel: int = None):
        ...
        self.shell.add_command(
            "downreport",
            cog="DownReportManager",
            description="Configure the down report manager",
        )
    ...
    async def initialize(self, force: bool = False):
        ...
        bot = self.bot.user
        avatar_url = bot.display_avatar.url if bot.display_avatar else None

        # Save the webhook url
        path = os.path.join(self.files.get_cache_dir(), "downreport-webhook.json")
        data = {"url": self.webhook.url, "avatar": avatar_url}
    ...
    async def get_webhook(self, force: bool = False) -> discord.Webhook:
        ...
        # Find the webhook
        webhook_name = f"{self.bot.user.name} | DownReport"
        for webhook in webhooks:
            if webhook.name == webhook_name:
                if force:
                    try:
                        await webhook.delete(
                            reason="Forced webhook recreation for downreport"
                        )
                    except discord.Forbidden:
        ...
        try:
            webhook = await channel.create_webhook(
                name=webhook_name, reason="Error reporting webhook"
            )
        except discord.Forbidden:
    ...
    async def shell_callback(self, command: ShellCommand):
        ...
        if command.name == "downreport":
            if command.query.startswith("get"):
                await command.log(
                    f"Webhook URL: {self.webhook.url}", title="Webhook URL"
                )
            elif command.query.startswith("set"):
                await command.log(
                    "Setting the webhook is not supported",
                    title="Error",
                    msg_type="error",
                )
            elif command.query.startswith("new"):
                message = await command.log(
                    "Creating a new webhook...", title="New Webhook"
                )
                await self.initialize(force=True)
                if self.webhook:
                    await command.log(
                        f"New webhook created: {self.webhook.url}",
                        title="New Webhook",
                        msg_type="success",
                        edit=message,
                    )
                    ...
                    await command.log(
                        "Failed to create a new webhook",
                        title="New Webhook",
                        msg_type="error",
                        edit=message,
                    )
...
def down_report(message: str):
    ...
    path = "config/cache/DownReportManager/downreport-webhook.json"
    ...
    # Send the message
    asyncio.run(_down_report_send_embed(url, embed, avatar_url=data.get("avatar")))


async def _down_report_send_embed(url: str, embed: discord.Embed, avatar_url: str = None):
    # Send the message
