...
class Database:
    ...
    def __init__(self, url: str, aerich: bool = True):
        self.url = url
        self.logger = get_framework_logger("db")
        self.active = False
        self._models = []
        if aerich:
            self.logger.info("Aerich is enabled.")
            self._models.append("aerich.models")
    ...
    async def init(self) -> None:
        ...
        if self.active:
            raise RuntimeError(
                "Database connection is already initialized. Cannot re-initialize until closed."
            )
        self.logger.info(
            f"Initializing database connection with {len(self._models)} models..."
        )
